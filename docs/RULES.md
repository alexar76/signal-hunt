...
- One round is keyed by the observation state hash (not the clock). A solved round stays
  frozen; an unplayed round may refresh if later snapshots change the diagnosis.
- Each session may submit once per round. A repeated request returns the stored verdict.
- The public mission card stays **sealed**: it discloses only baseline depth and a coarse
  *field tension* band (`low` / `elevated` / `high`) until a verdict exists. Severity and the
  detector class are revealed with the answer — publishing severity while sealed leaked
  `calm ⇒ stable` and collapsed the four-way choice.
- Field tension is the measured distance to the nearest threshold that did **not** trip, so it
  never names the class that did.
...
8. **Source concentration:** with at least two sources, the largest carries at least 60%
   of external capabilities **and that dominance is new versus the previous snapshot**.
   A first observation with no history may still fire. Persistent 60% is `stable`.
