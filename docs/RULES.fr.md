...
- La carte de mission publique reste **scellée** : jusqu’au verdict, elle ne publie que la
  profondeur de baseline et une bande grossière de *tension du champ* (`low` / `elevated` /
  `high`). La sévérité et la classe du détecteur sont révélées avec la réponse — publier la
  sévérité sous scellé livrait `calm ⇒ stable` et réduisait le choix de quatre à trois.
- La tension du champ est la distance mesurée au seuil le plus proche qui n’a **pas** été
  franchi ; elle ne nomme donc jamais la classe qui a déclenché.
