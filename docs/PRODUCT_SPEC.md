...
8. `source_concentration`: at least two external sources exist and the largest owns at
   least 60% of external capabilities, **and that source was not already at ≥60% on the
   most recent prior snapshot**. A first observation (no history) may still fire — it is a
   cross-sectional fact. Sitting at 60% for days is climate, not a new signal; the round
   then falls through to later detectors or `stable`;
...
Round identity is the measured field, not the clock: `round_id = sig_{sha256(state_hash)[:16]}`.
A stable catalogue does not mint a new puzzle every expiry bucket. `expires_at` still
advances on reuse. A **solved** round is immutable. An **unplayed** round may be replaced
in place (same id, new commitment) when later snapshots change history depth or detector
diagnosis — for example a first-observation `source_concentration` becoming `stable`
once the same 60% share is confirmed as climate.

The game process polls the Hub on `SIGNAL_HUNT_OBSERVE_INTERVAL_S` (compose default 300s)
so snapshot history exists even when nobody is playing. Snapshots are pruned to
`SIGNAL_HUNT_SNAPSHOT_KEEP`. Tests leave the interval at 0.

A round stores the observation state hash, diagnosis, ordered options and evidence
blocks. Before any player submits, the API publishes:
...
- committed rounds (solved immutable; unplayed may refresh on real history) and evidence trail;
