...
GitHub Actions 会运行 Signal Hunt 的 pytest、前端构建，以及针对 `env.ci` 的 `docker compose config`。
DIOSCURI hero feed 的签名契约由 monorepo 中 DIOSCURI 包的测试（`dioscuri/test/signal-hunt-heroes.test.ts`）覆盖。项目使用
