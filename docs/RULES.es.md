...
- La tarjeta de misión pública permanece **sellada**: solo profundidad del baseline y una banda
  gruesa de *tensión de campo* (`low` / `elevated` / `high`) hasta el veredicto. La severity y la
  clase del detector se revelan con la respuesta — publicar la severity sellada filtraba
  `calm ⇒ stable` y reducía la elección de cuatro a tres.
- La tensión de campo es la distancia medida al umbral más cercano que **no** se cruzó, así que
  nunca nombra la clase que sí lo hizo.
