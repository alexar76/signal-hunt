...
export default function App() {
  ...
  const [secondLockWarned, setSecondLockWarned] = useState(false);
  ...
  const followUpLabel = (option: string) => {
    ...
    // closest_rule reuses the round's diagnosis codes as its options.
    const diagnosisKey = `diagnosis.${option}`;
    const named = t(diagnosisKey);
    return named === diagnosisKey ? compactHost(option) : named;
  };
  ...
  const severityLabel = round
    ? (round.signal.severity === 'calm'
      ? t('labelCalm')
      : round.signal.severity === 'unsettled'
        ? t('labelUnsettled')
        : round.signal.severity === 'sealed'
          ? t('labelSealed')
          : t('labelAnomaly'))
    : '';
  ...
  const submit = async () => {
    ...
    // The second lock is free points; players kept committing straight past it.
    if (round.follow_up && !followUp && !secondLockWarned) {
      setSecondLockWarned(true);
      return;
    }
        ...
        <section className="mission-section" id="mission">
          ...
          <div className="mission-grid">
            <article className={`signal-card severity-${round.signal.severity}`}>
              ...
              {round.signal.tension && (
                <div className={`tension-card band-${round.signal.tension.band}`}>
                  <small>{t('tensionTitle')}</small>
                  <strong>{t(`tension.${round.signal.tension.band}`)}</strong>
                  <p>{t('tensionHelp')}</p>
                </div>
              )}
        ...
        <section className="verdict-section">
          ...
          {!verdict ? <div className="verdict-console">
              ...
              {round.follow_up && (
                <div className="follow-up-panel">
                  <div className="follow-up-heading">
                    <small>{t('followUpTitle')}<b className="lock-value">{t('secondLockValue')}</b></small>
                  ...
                  {secondLockWarned && !followUp && (
                    <p className="follow-up-warn" role="status">{t('secondLockWarn')}</p>
                  )}
                </div>
              )}
            </div>
            <div className="confidence-panel"><div><span>{t('confidence')}</span><strong>{Math.round(confidence * 100)}%</strong></div>
              ...
              <button className="submit-button" disabled={!selected || submitting} onClick={submit}>{
                submitting
                  ? t('submitting')
                  : (secondLockWarned && round.follow_up && !followUp ? t('secondLockCommitAnyway') : t('submit'))
              }<span>→</span></button>
            ...
            {Boolean(round.signal.near_misses?.length) && (
              <div className="near-miss-card">
                <small>{t('nearMissTitle')}</small>
                <p>{t('nearMissHelp')}</p>
                <div className="near-miss-rows">
                  {round.signal.near_misses!.map((miss) => (
                    <div className="near-miss-row" key={miss.code}>
                      <strong>{t(`diagnosis.${miss.code}`)}</strong>
                      <div className="near-miss-bar"><i style={{ width: `${Math.min(100, miss.closeness * 100)}%` }} /></div>
                      <b>{t('nearMissCloseness', { pct: Math.round(miss.closeness * 100) })}</b>
                    </div>
                  ))}
                </div>
              </div>
            )}
            {round.engagement?.field && (
              <div className="field-card">
                <small>{t('fieldTitle')}</small>
                {round.engagement.field.submissions <= 1
                  ? <strong>{t('fieldAlone')}</strong>
                  : <>
                    <strong>{t('fieldVerdicts', { count: round.engagement.field.submissions })}</strong>
                    <span>{t('fieldAccuracy', { pct: Math.round((round.engagement.field.accuracy || 0) * 100) })}</span>
                    <span>{t('fieldAgreed', { count: round.engagement.field.your_answer_votes || 0 })}</span>
                  </>}
              </div>
            )}
