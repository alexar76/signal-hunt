...
def _closest_rule_follow_up(
    diagnosis: str,
    margins: list[dict[str, Any]] | None,
    options: list[str] | None,
) -> tuple[dict[str, Any], dict[str, Any]] | None:
    """On a quiet round, ask which rule came closest to firing instead.

    A stable reading used to draw the generic band question, so the whole round was
    one lookup. This couples the second lock to the same margin reasoning the main
    verdict needs — and it reuses the round's own option set, so comparing the two
    lists can never point at the sealed answer.
    """
    if diagnosis != "stable" or not options:
        return None
    ranked = [row for row in (margins or []) if str(row.get("code") or "") in options]
    if len(ranked) < 2:
        return None
    top, second = ranked[0], ranked[1]
    # A tie has no single right answer — fall back rather than ask a coin flip.
    if float(top.get("closeness") or 0) <= float(second.get("closeness") or 0):
        return None
    public = {
        "id": "follow_up",
        "kind": "closest_rule",
        "prompt": "followUp.closest_rule",
...
def build_follow_up(
    ...
    *,
    ...
    near_miss = _closest_rule_follow_up(diagnosis, margins, options)
