...
def detect(
    ...
    if len(sources) >= 2:
        leader = max(sources, key=lambda row: row.get("capabilities", 0))
        share = leader.get("share")
        if isinstance(share, (int, float)) and share >= 0.60:
...
def build_round(
    ...
    *,
    ...
    round_seed = hashlib.sha256(
        f"{bucket}:{current['state_hash']}:{diagnosis}".encode()
    ).hexdigest()
    round_id = f"sig_{round_seed[:16]}"
