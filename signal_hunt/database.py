...
class GameDatabase:
    ...
    def round_field_tally(self, round_id: str) -> dict[str, Any]:
        """What the whole field answered on one round — no handles, counts only.

        A three-player leaderboard gives a player nothing to measure themselves
        against; the round's own answer spread does, even with three players.
        """
        with self._lock:
            rows = self._db.execute(
                "SELECT answer_code, COUNT(*) AS votes, SUM(correct) AS hits "
                "FROM submissions WHERE round_id=? GROUP BY answer_code",
                (round_id,),
            ).fetchall()
        options = {str(row["answer_code"]): int(row["votes"]) for row in rows}
        total = sum(options.values())
        correct = sum(int(row["hits"] or 0) for row in rows)
        return {
            "submissions": total,
            "correct": correct,
