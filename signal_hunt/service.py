...
class GameService:
    ...
    async def run_observer(self) -> None:
        """Poll the Hub on a timer so history exists even when nobody is playing."""
        interval = self.settings.observe_interval_s
        if interval <= 0:
            return
        keep = self.settings.snapshot_keep
        while True:
            try:
                await self.observation()
                self.db.prune_snapshots(keep)
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("signal-hunt observer snapshot failed")
    ...
    async def live_round(self, session_id: str) -> dict[str, Any]:
        ...
        existing = self.db.get_round(provisional["id"]) or self.db.latest_round_for_state_hash(
            current["state_hash"]
        )
        if existing is not None:
            ...
            solved = self.db.round_solve_count(existing["id"]) > 0
            history_grew = len(history) > sealed_depth
            diagnosis_changed = existing.get("diagnosis_code") != provisional["diagnosis_code"]
            # Unplayed rounds may refresh when history grows or the detector
            # now disagrees (level concentration becoming stable). Solved stays frozen.
            if solved or (not history_grew and not diagnosis_changed):
                period = max(self.settings.round_seconds, 60)
                bucket = int(now.timestamp()) // period
                expires = datetime.fromtimestamp((bucket + 1) * period, tz=UTC)
                self.db.extend_round_expiry(
                    existing["id"], expires.isoformat().replace("+00:00", "Z")
                )
                record = self.db.get_round(existing["id"])
                assert record is not None
                return self._public_round(record, session_id)
        assist = await self.federation.fairness_seed(
            f"signal-hunt:{current['state_hash']}"
        )
        built = build_round(
            current,
            history,
        ...
        if existing is not None:
            ...
            built["payload"]["id"] = existing["id"]
            built["answer_commitment"] = hashlib.sha256(
                f"{built['id']}:{built['diagnosis_code']}:{built['answer_salt']}".encode()
            ).hexdigest()
            built["payload"]["answer_commitment"] = built["answer_commitment"]
    ...
    def _public_round(self, record: dict[str, Any], session_id: str) -> dict[str, Any]:
        ...
        stored_signal = payload.get("signal") if isinstance(payload.get("signal"), dict) else {}
        tension = stored_signal.get("tension")
        if not isinstance(tension, dict) and isinstance(reveal, dict):
            candidate = reveal.get("tension")
            tension = candidate if isinstance(candidate, dict) else None
        if submitted and reveal is not None:
            payload["signal"] = {
                "sealed": False,
                "severity": reveal.get("severity")
                or ("calm" if reveal["code"] == "stable" else "anomaly"),
                "history_depth": stored_signal.get("history_depth"),
                "tension": tension,
                # Debrief: how close the fields that did not fire actually came.
                "near_misses": reveal.get("near_misses") or [],
                "code": reveal["code"],
            ...
            payload["signal"] = {
                "sealed": True,
                # Severity is part of the answer: "calm" meant "stable" and turned a
                # 4-way choice into a 3-way one. Sealed rounds publish tension only.
                "severity": "sealed",
                "tension": tension,
        ...
        if submitted:
            ...
            # The answer spread is a spoiler until this player has committed.
            tally = self.db.round_field_tally(record["id"])
            picked = str(submitted.get("selected") or "")
            payload["engagement"]["field"] = {
                **tally,
                "your_answer": picked or None,
                "your_answer_votes": int(tally["options"].get(picked, 0)),
            }
