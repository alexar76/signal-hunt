...
from signal_hunt.service import GameService
...
async def test_round_evidence_submit_and_idempotency(tmp_path):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        ...
        # Severity is part of the answer — a sealed card must not ship it.
        assert round_data["signal"]["severity"] == "sealed"
        assert round_data["signal"]["tension"]["band"] in {"low", "elevated", "high"}
        ...
        assert revealed["signal"]["severity"] == "anomaly"
        assert isinstance(revealed["signal"]["near_misses"], list)
        # The field spread only exists once this player has committed.
        field = revealed["engagement"]["field"]
        assert field["submissions"] == 1
        assert field["correct"] == 1
        assert field["accuracy"] == 1.0
        assert field["your_answer"] == "source_concentration"
        assert field["options"] == {"source_concentration": 1}
        assert "field" not in round_data.get("engagement", {})
...
async def test_unplayed_round_refreshes_when_history_grows(tmp_path):
    ...
    async with httpx.AsyncClient(
        ...
        record = db.get_round(second["id"])
        assert record is not None
        assert record["diagnosis_code"] == "stable"


class CountingFederation:
    ...
    async def snapshot(self):
        self.n += 1
        snap = live_snapshot()
        snap["observation_id"] = f"obs-count-{self.n}"
        snap["observed_at"] = f"2026-08-10T10:00:{self.n:02d}Z"
        return snap

    async def fairness_seed(self, alpha: str):
        return await FixedFederation().fairness_seed(alpha)


async def test_observer_writes_snapshots_without_a_player(tmp_path):
    settings = Settings(
        data_dir=tmp_path,
        hub_url="https://hunt.example",
        observation_cache_s=0,
        observe_interval_s=0.02,
        snapshot_keep=50,
    )
    db = GameDatabase(":memory:")
    service = GameService(
        ...
        db,
        CountingFederation(),
        SessionTokens(tmp_path / "sessions-obs"),
    )
    task = asyncio.create_task(service.run_observer())
    try:
        for _ in range(40):
            if db.snapshot_count() >= 2:
                break
...
def test_prune_snapshots_keeps_newest():
    db = GameDatabase(":memory:")
    for index in range(5):
        snap = live_snapshot()
        snap["observation_id"] = f"obs-prune-{index}"
        snap["observed_at"] = f"2026-08-10T10:0{index}:00Z"
        db.save_snapshot(snap)
    assert db.prune_snapshots(2) == 3
    assert db.snapshot_count() == 2
    remaining = {row["observation_id"] for row in db.recent_snapshots(10)}
    assert remaining == {"obs-prune-3", "obs-prune-4"}


async def test_health_reports_observer_settings(tmp_path):
    settings = Settings(
        data_dir=tmp_path,
        hub_url="https://hunt.example",
        observe_interval_s=0,
    )
    app = create_app(
        settings,
        db=GameDatabase(":memory:"),
        federation=FixedFederation(),
        tokens=SessionTokens(tmp_path / "sessions-health"),
        signer=ProviderSigner(tmp_path / "provider-health"),
    )
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=app), base_url="http://test"
    ) as client:
        body = (await client.get("/health")).json()
        assert body["data_mode"] == "live-only"
        assert body["observe_interval_s"] == 0
