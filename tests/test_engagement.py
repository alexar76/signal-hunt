...
def test_quiet_round_asks_which_rule_came_closest():
    margins = [
        {"code": "source_concentration", "closeness": 0.92, "detail": {"share_pct": 55.0}},
        {"code": "catalog_expansion", "closeness": 0.66, "detail": {"change": 2}},
        {"code": "latency_weather", "closeness": 0.2, "detail": {"max_ms": 100.0}},
    ]
    options = ["stable", "source_concentration", "catalog_expansion", "latency_weather"]
    public, reveal = build_follow_up(
        {"capabilities": {"external": 53}, "prices": {}, "sources": []},
        [],
        diagnosis="stable",
        round_seed="round-1",
        margins=margins,
    ...
    assert public["kind"] == "closest_rule"
    assert reveal["answer"] == "source_concentration"
    # Same option set as the verdict — the two lists cannot be diffed for the answer.
    assert public["options"] == options


def test_closest_rule_falls_back_on_a_tie_and_on_events():
    tied = [
        {"code": "source_concentration", "closeness": 0.5, "detail": {}},
        {"code": "catalog_expansion", "closeness": 0.5, "detail": {}},
    ]
    options = ["stable", "source_concentration", "catalog_expansion", "price_shift"]
    current = {
        "capabilities": {"external": 53},
        "prices": {"median_usd": 0.01},
        "sources": [
    ...
    public, _ = build_follow_up(
        current, [], diagnosis="stable", round_seed="r", margins=tied, options=options
    )
    assert public["kind"] != "closest_rule"
    ...
    public, _ = build_follow_up(
        current,
        [],
        diagnosis="source_concentration",
        round_seed="r",
        margins=[
            {"code": "catalog_expansion", "closeness": 0.9, "detail": {}},
            {"code": "price_shift", "closeness": 0.1, "detail": {}},
        ],
        options=options,
    )
    assert public["kind"] == "leading_source"
