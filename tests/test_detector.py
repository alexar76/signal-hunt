...
from signal_hunt.detector import (
    build_round,
    detect,
    rule_margins,
    score_answer,
...
def test_persistent_concentration_is_climate_not_a_new_signal():
    history = []
    for index in range(4):
        row = snapshot(state=str(index), sources={"oracle": 42, "iot": 11})
        row["observed_at"] = f"2026-08-10T10:0{index}:00Z"
        history.append(row)
    code, _params = detect(snapshot(state="now", sources={"oracle": 42, "iot": 11}), history)
    assert code == "stable"


def test_concentration_fires_when_share_newly_crosses_threshold():
    history = [snapshot(state="calm", sources={"oracle": 28, "iot": 25})]
    code, params = detect(
        snapshot(state="hot", sources={"oracle": 42, "iot": 11}), history
    )
    assert code == "source_concentration"
    assert params["source_hub"] == "oracle"


def test_same_field_hash_reuses_round_id_across_clock_buckets():
    current = snapshot(state="a", sources={"oracle": 42, "iot": 11})
    first = build_round(current, [], now=datetime(2026, 8, 10, 10, 15, tzinfo=UTC))
    later = build_round(current, [], now=datetime(2026, 8, 10, 11, 15, tzinfo=UTC))
    assert first["id"] == later["id"]
    assert first["id"].startswith("sig_")


def test_rule_margins_rank_the_closest_unfired_rule_first():
    # Two sources at 55/45 sit just under the 60% concentration gate; the catalog
    # moved +2 of the +3 it needs, so concentration must outrank expansion.
    peer = {"url": "https://p1", "name": "p1", "latency_ms": 100.0, "capabilities_count": 4}
    history = [
        snapshot(state="h", sources={"oracle": 27, "iot": 24}, peers=[peer])
        for _ in range(3)
    ]
    current = snapshot(state="c", sources={"oracle": 29, "iot": 24}, peers=[peer])
    code, _ = detect(current, history)
    assert code == "stable"
    margins = rule_margins(current, history, firing=code)
    codes = [row["code"] for row in margins]
    assert codes[0] == "source_concentration"
    assert "catalog_expansion" in codes
    assert "stable" not in codes
    assert all(0 < row["closeness"] <= 1 for row in margins)
    # latency margin is honest: 100 ms of a 500 ms threshold.
    latency = next(row for row in margins if row["code"] == "latency_weather")
    assert latency["closeness"] == 0.2


def test_options_are_near_misses_not_hash_noise():
    peer = {"url": "https://p1", "name": "p1", "latency_ms": 100.0, "capabilities_count": 4}
    history = [
        snapshot(state="h", sources={"oracle": 27, "iot": 24}, peers=[peer])
        for _ in range(3)
    ...
    current = snapshot(state="c", sources={"oracle": 29, "iot": 24}, peers=[peer])
    record = build_round(current, history, now=datetime(2026, 8, 10, 10, 30, tzinfo=UTC))
    options = record["payload"]["options"]
    assert record["diagnosis_code"] == "stable"
    assert "stable" in options
    margins = rule_margins(current, history, firing="stable")
    assert set(options) - {"stable"} == {row["code"] for row in margins[:3]}


def test_sealed_round_never_publishes_severity_or_margins():
    record = build_round(
        snapshot(state="c", sources={"oracle": 20, "iot": 20}),
        [snapshot(state="h", sources={"oracle": 40, "iot": 40})] * 3,
        now=datetime(2026, 8, 10, 10, 30, tzinfo=UTC),
    )
    assert record["diagnosis_code"] == "catalog_contraction"
    signal = record["payload"]["signal"]
    assert signal["sealed"] is True
    assert signal["severity"] == "sealed"
    assert signal["tension"]["band"] in {"low", "elevated", "high"}
    assert "near_misses" not in signal
    reveal = record["payload"]["_signal_reveal"]
    assert reveal["severity"] == "anomaly"
    assert reveal["near_misses"]


def test_quiet_round_near_a_threshold_reads_unsettled():
    history = [snapshot(state="h", sources={"oracle": 27, "iot": 24}) for _ in range(3)]
    current = snapshot(state="c", sources={"oracle": 29, "iot": 24})
    calm = build_round(current, history, now=datetime(2026, 8, 10, 10, 30, tzinfo=UTC))
    assert calm["diagnosis_code"] == "stable"
    assert calm["payload"]["_signal_reveal"]["severity"] == "unsettled"

    far = snapshot(state="d", sources={"oracle": 9, "iot": 9, "mesh": 9, "edge": 9})
    quiet = build_round(
        far,
        [snapshot(state="e", sources={"oracle": 9, "iot": 9, "mesh": 9, "edge": 9})] * 3,
        now=datetime(2026, 8, 10, 10, 30, tzinfo=UTC),
    )
    assert quiet["diagnosis_code"] == "stable"
    assert quiet["payload"]["_signal_reveal"]["severity"] == "calm"
